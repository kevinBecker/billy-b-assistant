...
# Install TTS service compatible with Python 3.13+
# This script automatically chooses the best TTS solution for your Python version

set -e

echo "🚀 Installing TTS service for Python 3.13+ compatibility..."
...
def health_check():
    ...
    return jsonify({"status": "healthy", "engine": "espeak-ng", "python": "3.13+"})
...
if __name__ == '__main__':
    logger.info("Starting Simple TTS server on port 5002 (Python 3.13+ compatible)")
...
[Unit]
Description=Simple TTS Server for Billy B-Assistant (Python 3.13+ Compatible)
